...
func (ctl *Control) handleNatHoleVisitor(m msg.Message) {
	...
	ctl.sessionCtx.RC.NatHoleController.HandleVisitor(ctl.ctx, inMsg, ctl.msgTransporter, ctl.sessionCtx.LoginMsg.User)
}

func (ctl *Control) handleNatHoleClient(m msg.Message) {
	inMsg := m.(*msg.NatHoleClient)
	ctl.sessionCtx.RC.NatHoleController.HandleClient(ctl.ctx, inMsg, ctl.msgTransporter)
}

func (ctl *Control) handleNatHoleReport(m msg.Message) {
	inMsg := m.(*msg.NatHoleReport)
	ctl.sessionCtx.RC.NatHoleController.HandleReport(ctl.ctx, inMsg)
}
